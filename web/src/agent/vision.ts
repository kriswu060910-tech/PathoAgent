...
export async function analyzeImage(image: ImageAttachment, prompt?: string): Promise<string> {
  ...
  if (!baseUrl || !apiKey) {
    throw new Error('视觉服务未配置，请在 .env 中设置 VITE_VISION_BASE_URL 和 VITE_VISION_API_KEY。')
  }
...
export async function detectObjects(
  ...
  const raw = await analyzeImage(image, prompt)
  try {
...
export async function detectObjectsWithEdges(
  ...
  // 第一步：LLM 检测（获取标签 + 粗略框）
  const llmBoxes = await detectObjects(image, customPrompt)
...
export async function analyzeImages(images: ImageAttachment[], prompt?: string): Promise<string> {
  ...
  const descriptions = await Promise.all(
    images.map(async (img, i) => {
      const desc = await analyzeImage(img, prompt)
